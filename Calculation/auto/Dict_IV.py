...
def read_csv_to_dict(filepath):
    """
    Reads a CSV file with two header rows, filters by Valid QC, and stores data in a 3D coordinate-based nested dictionary:
    data_dict[Well_ID][Sweep_Num][Feature] = Value
    """
    ...
    for idx, well_id in enumerate(df.index):
        ...
        for sweep_num in sweeps:
            ...
            # Store features for each sweep
            for col in [c for c in df.columns if sweep_pattern.search(c[0]) and sweep_pattern.search(c[0]).group(1) == sweep_num[-3:]]:
                # Remove non-alphanumeric characters and trailing numbers from feature names
                feature = re.sub(r'[^a-zA-Z0-9]', '', col[1]).lower()
                feature = re.sub(r'\d+$', '', feature)  # Remove trailing numbers
                value = df.iloc[idx][col]
                data_dict[current_well][sweep_num][feature] = value
                # print(f"Storing: Well={current_well}, Sweep={sweep_num}, Feature={feature}, Value={value}")

    return data_dict


def get_value(data_dict, well_id, sweep, feature):
    """Retrieve a specific value from the 3D dictionary."""
    # print(f"Retrieving: Well={well_id}, Sweep={sweep}, Feature={feature}")
    sweep_formatted = re.sub(r'\\s+', '', sweep)
    feature_formatted = re.sub(r'[^a-zA-Z0-9]', '', feature).lower()  # Ensure formatting matches storage
...
def get_values_across_sweeps(data_dict, well_id, feature):
    """Retrieve a list of values for a specific feature across all sweeps of a well."""
...
def process_all_files():
    """Process all CSV files and store the dictionaries."""
    all_data = {}
    for filepath in glob.glob("*IV*.csv"):
...
if __name__ == "__main__":
    ...
    if all_data:
        ...
        print(get_value(all_data[first_file], 'D01', 'Sweep001', 'TP1CurDen'))
        print(get_values_across_sweeps(all_data[first_file], 'D01', 'TP1CurDen'))
